...
### 流量控制
TCP为应用程序提供了**流量控制服务(flow-control service)**来消除发送方使接收方缓存溢出的可能性. 流量控制目的是匹配发送方的发送速率与接受方app的读取速率. TCP靠两端的发送方**接收窗口(receive window)**来提供流量控制, 其指示了接收方还有多少可用的缓存空间. 在传输文件的例子中, 我们仅研究接收窗口. A通过TCP向B发送大文件, B为该文件分配大小为RcvBuffer的接收缓存. 定义``LastByteRead``是B上的app从缓存读出的数据流的最后一个字节的编号, ``LastByteRcvd``是已放入B接收缓存的术交流的最后一个字节的编号. 接收窗口``rwnd = RcvBuffer - [LastByteRcvd - LastByteRead]``.
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_11.png)

而A主要监控LastByteSent - LastByteAcked, 是其小于rwnd. 但是如果B缓存已满, 发了rwnd=0给A, 并且后面B没有数据发给A, 那么就会导致A阻塞, 为了解决这个问题, 即使rwnd为0, A依然会向B发送1个字节的报文段, b收到后清空缓存并ack, 告诉A rwnd值.
...
3. C位该链接分配缓存和变量. 返回给服务器端一个报文段. 这里的Syn被设置为0. 并且这个阶段可以在报文段负载中携带C到SDE数据.
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_12.png)

#### 四!次!挥!手!
1. C向S发送报文段, 其中FIN为1. 进入FIN_WAIT_1状态.
2. S返回ACK表示收到了该报文段. C进入FIN_WAIT_2状态
3. S关闭后向C发送终止报文段, 其中FIN为1.
4. C向S返回ACK, 进入TIME_WAIT状态. 如果ACK丢失, C会重传.
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_13.png)
...
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_14.png)
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_15.png)
...
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_16.png)
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_17.png)

3. 4个发送方和多个有缓存的路由器组层的多跳路径. **这里的代价是一个分组在一条路径被丢弃, 上游路由器从转发到丢弃次分组而使用的传输容量被浪费**.
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_18.png)
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_19.png)

### 阻塞控制方法
**todo**

### TCP阻塞控制
TCP能根据网络的阻塞程度, 通过**阻塞窗口(cwnd)**来限制发送流量的速率, 在后面的例子中, 我们假设TCP接收缓存足够大.  
...
1. 超时指示的丢包, 将ssthresh设置为cwnd/2.
2. 冗余的ACK, ssthresh设置为cwnd/2, wnd减小为之前的一半, 进入**快速恢复状态**.

### 快速恢复
对收到的每个冗余的ACK, cwnd增加一个MSS. 当丢失报文段的ACK到达后, TCP就在降低cwnd之后进入阻塞避免状态. 如果出现超时, 那么cwnd又会从1开始重新慢启动, 并且设置ssthresh设置为cwnd/2. 对于冗余的ACK, 注意Tahoe和Reno的区别, 较早的Tahoe无论超时/3个冗余都会重新慢启动:
![](https://github.com/huanruiz/PunchingCS/blob/master/images/cn3_20.png)
