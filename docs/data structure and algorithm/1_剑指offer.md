这是剑指offer第二版的题目小总结, 对应leetcode上面的题目. 题目名字的格式为: ``编号.题目名(leetcode主站编号)``.
...
## 链表
### 06.从尾到头打印链表
1.栈辅助, 但是动态增加长度可能会消耗多余空间, 直接遍历得到长度并用固定长度数组也不失为一种好办法; 对python直接添加到list再reverse也可以. 2. 回溯. 两种方法都是双O(N).
...
### 25.合并两个排序的链表(21)
1.建个新的链表遍历 2.递归, 哪个小就返回哪个, 递归避免了新建额外的链表, 归函数的功能是返回已经merge好的链表.

### 24.反转链表(206)
1.双指针, 用两个相邻指针遍历数组. 2.递归, 天然的用上一层来记录各个节点位置, 递
```
...
### 52.两个链表的第一个公共节点
1.我首先想到的就是遍历链表A再把node存起来, 再遍历链表B比较; 2.浪漫的做法是双指针, 遍历A+B两个链表的总步数一样, 如果有相同的小尾巴, 那么双指针一定会相遇, 主要节省了空间复杂度O(1).

### 62.圆圈中最后剩下的数字
1.暴力模拟, 用来记录每次的index, 这里结果要对m取模, 防止计算出来的i 大于等于剩下的list的长度; 2.约瑟夫环, 递推公式: $(n, m) = [f(n−1, m)+m]\%n$, 当$n = 1$时, $f(1, m) = 0$.
...
## 递归&动态规划
大多数都是动态规划的题. dp主要是自底向上解决问题, 而递归是暴力的自顶向下, 一般会超时. 所以如果用递归的话需要记忆化来存储已经计算的值, 并且剪枝. 动态规划也可以优化, 大多是优化空间, 用几个值来代替数组, 存储上几个状态的值.
...
### 19.正则表达式匹配(10)
回溯来做. 先判断s和p的第一个字符是否匹配, ``p[1]为*``: 匹配0个或多个字符, ``p[1]为.``: 匹配一个字符.
