...
## 归并排序
归并排序用了分治的思想, 和堆排序一样, 利用了完全二叉树. 改进了冒泡排序(减少循环次数), 需要进行logn次循环, 每一趟都会进行n次扫描, 所以时间复杂度总是保持在$nlogn$. **经典的逆序对问题其实就可以通过归并排序的思想来做**.
```
// Java
...
## 桶排序
思路同计数排序, 但是把数字划分到不同的桶中, 再对桶内部排序.

## 基数排序
根据从个位到十位, 十位到百位进行比较, 把数字放进对应的index中, 最后填回序列的时候, 整个序列就自动地排好了序.
```
# Python (注意此方法仅针对正数)
bucket = [[0 for i in range(len(arr))] for i in range(10)]
bucket_count = [0 for i in range(10)]
j = 0
n = 1
while j < len(arr):
